...
def get():
	'''
	objectIDに対応する作品情報の取得
	'''
	# パラメータからobjectIDを取得
	objectID = request.args.get('objectID', default=None)
	# 必要な情報を取得する
	json_data = APIGoogleBooks().get_json(objectID) if objectID else {}
...
def regist():
	...
	if len(json_data) > 0:
		# Elasticsearch
		es = ElasticsearchWrapper('metmuseum', 'metmuseum-index')
		# 追加
...
def search():
	...
	# Elasticsearch
	es = ElasticsearchWrapper('meymuseum', 'meumuseum-index')
	# 検索
